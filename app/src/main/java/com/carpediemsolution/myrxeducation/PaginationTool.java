...
public class PaginationTool<T> {
    ...
    // первый раз items loading
    private static final int EMPTY_LIST_ITEMS_COUNT = 0;
    // default limit для запросов
    private static final int DEFAULT_LIMIT = 50;
    // default max attempts для попыток повторения запросов
    ...
    public Observable<T> getPagingObservable() {
        ...
        return getScrollObservable(recyclerView, limit, emptyListCount)
                ...
                .switchMap(offset -> getPagingObservable(pagingListener, pagingListener.onNextPage(offset+1), startNumberOfRetryAttempt, offset, retryCount));
    }
                ...
                public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                    if (!subscriber.isUnsubscribed()) {
                        ...
                        if (position >= updatePosition) {
                            int offset = emptyListCountPlusToOffset ? recyclerView.getAdapter()
                                    .getItemCount() : recyclerView.getAdapter()
    ...
    private int getLastVisibleItemPosition(RecyclerView recyclerView) {
        ...
        throw new PagingException("неизвестный LayoutManager класс: " + recyclerViewLMClass.toString());
    }
    ...
    public static class Builder<T> {
        ...
        public Builder<T> setLimit(int limit) {
            if (limit <= 0) {
                throw new PagingException("limit д.б. больше 0");
            }
            this.limit = limit;
            return this;
        }
        ...
        public Builder<T> setEmptyListCount(int emptyListCount) {
            if (emptyListCount < 0) {
                throw new PagingException("emptyListCount д.б. не меньше 0");
            }
        ...
        public Builder<T> setRetryCount(int retryCount) {
            if (retryCount < 0) {
                throw new PagingException("retryCount д.б. не меньше 0");
            }
